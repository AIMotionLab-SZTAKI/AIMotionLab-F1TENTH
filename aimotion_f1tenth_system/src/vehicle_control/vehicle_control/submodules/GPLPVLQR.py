...
class GPLPVLQR(BaseController):
    def __init__(self, FREQUENCY, 
        ...
        super(GPLPVLQR, self).__init__(FREQUENCY=FREQUENCY, car_id= vehicle_id)
        ...
        self.get_logger().info("GP-LPV-LQR trajectory-tacking controller initialized!")

    def execute_trajectory(self, trajectory_request, response):
        self.get_logger().info("Trajectory execution request recieved!")
        super(GPLPVLQR,self).execute_trajectory(trajectory_request, response)
        self.q=0
    ...
    def state_callback(self, data):
        ...
        super(GPLPVLQR,self).state_callback(data)
        ...
        ### PROCESS STATE DATA & evaluae trajectory ###    
        # get velocity data
        ...
        # project
        self.s=project_to_closest(pos=position,
                                  s_est=s_ref,
                                  s_window=self.projection_window,
                                  path=self.get_path,
                                  step=self.projection_step,
                                  s_bounds=(self.s_start,self.s_end))
        ...
        if (v_ref > 0 and d < 0) or (v_ref < 0 and d > 0): d = 0
